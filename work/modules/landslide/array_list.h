...
#ifndef __LS_ARRAY_LIST_H
#define __LS_ARRAY_LIST_H

#include "common.h" /* MM_XMALLOC / MM_FREE */
#include "compiler.h"
...
#define ARRAY_LIST_INIT(a, initial_capacity) do {				\
		...
		__a->array = MM_XMALLOC(__a->capacity, typeof(*__a->array));	\
	} while (0)

#define ARRAY_LIST_FREE(a) MM_FREE((a)->array)
...
#define ARRAY_LIST_CLONE(dest, src) do {					\
		...
		__dest->array = MM_XMALLOC(__src->size, typeof(*__src->array));	\
...
#define ARRAY_LIST_APPEND(a, val) do {					\
		...
		if (__a->size == __a->capacity) {			\
			...
			__a->capacity *= 2;				\
			__a->array = MM_XMALLOC(__a->capacity,		\
						typeof(*__a->array));	\
